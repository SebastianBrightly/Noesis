...
export class RAGService {
	...
	private pendingManualRetry: boolean = false;
		/** Number of automatic embedding-retry attempts performed so far (persisted across pause calls)
		 * This prevents repeated calls to automatic retry loops when pauseForEmbeddingRecovery
		 * is invoked multiple times in quick succession.
		 */
		private embeddingAutoRetryAttempts: number = 0;
	...
	initializeImageTextExtractor(llmService: LLMService, pluginSettings?: LocalLLMSettings): void {
		try {
			...
			// If plugin settings are provided, configure extractor behavior
			if (pluginSettings && this.imageTextExtractor && typeof pluginSettings.enableLocalOCRFallback !== 'undefined') {
				this.imageTextExtractor.setEnableLocalOCRFallback(!!pluginSettings.enableLocalOCRFallback);
			}
	...
	retryPausedIndexing(): boolean {
		...
		// Reset auto-retry counter when user requests a manual retry
		this.embeddingAutoRetryAttempts = 0;
	...
	private normalizeHeadingSlug(value: string): string {
		return value
			.toLowerCase()
			.trim()
			.replace(/\[\[([^\]|]+)\|?([^\]]+)?\]\]/g, '$2$1')
	...
	private splitMarkdownIntoAnchoredChunks(content: string): ChunkContent[] {
		let cleanContent = content;
		const frontmatterMatch = content.match(/^---\s*\n([\s\S]*?)\n---\s*\n/);
		if (frontmatterMatch) {
			cleanContent = content.substring(frontmatterMatch[0].length);
		...
		const headingStack: Array<{ level: number; text: string; slug: string }> = [];
		...
		const finalizeChunk = () => {
			...
			const wordCount = cleanChunk.split(/\s+/).filter(Boolean).length;
			if (wordCount < 10) {
				currentChunk = '';
				currentWordCount = 0;
				return;
			...
			const blockMatch = cleanChunk.match(/\^([a-zA-Z0-9-]{2,})\b/);
			const blockId = blockMatch?.[1];
			const resolvedAnchorType: 'block' | 'heading' | 'chunk' = blockId ? 'block' : (headingSlug ? 'heading' : 'chunk');
			const resolvedAnchorValue = blockId || headingSlug || `c${chunks.length}`;
			const resolvedAnchorTarget = blockId
				? `#^${blockId}`
				: (headingSlug ? `#${headingSlug}` : `#c${chunks.length}`);
			...
			chunks.push({
				...
				anchorTarget: resolvedAnchorTarget,
				headingPath: headingPath.length > 0 ? headingPath.join(' > ') : undefined,
				headingSlug
			});
		...
		for (let i = 0; i < lines.length; i++) {
			...
			if (headingMatch) {
				...
				const slug = this.normalizeHeadingSlug(text);
			...
			if (currentWordCount > maxWords) {
				...
				if (cleanChunk.length > 0) {
					const splitChunks = this.splitLongChunk(cleanChunk, maxWords);
					for (const splitChunk of splitChunks) {
						const blockMatch = splitChunk.match(/\^([a-zA-Z0-9-]{2,})\b/);
						const blockId = blockMatch?.[1];
						chunks.push({
							text: splitChunk,
							index: chunks.length,
							sectionIndex,
							anchorType: blockId ? 'block' : (headingSlug ? 'heading' : 'chunk'),
							anchorValue: blockId || headingSlug || `c${chunks.length}`,
							anchorTarget: blockId ? `#^${blockId}` : (headingSlug ? `#${headingSlug}` : `#c${chunks.length}`),
							headingPath: headingPath.length > 0 ? headingPath.join(' > ') : undefined,
							headingSlug
						});
					}
		...
		finalizeChunk();
		LoggingUtility.log(`Split markdown into ${chunks.length} anchored chunks`);
		return chunks;
	}
	...
	private clearEmbeddingPauseState(): void {
		...
		// Reset auto-retry attempts when clearing the paused state
		this.embeddingAutoRetryAttempts = 0;
	}
	...
	private async pauseForEmbeddingRecovery(reason: string): Promise<void> {
		...
		if (category !== 'vision') {
			...
			// If we've already performed some auto-retries previously, resume from that count
			for (let attemptIdx = this.embeddingAutoRetryAttempts; attemptIdx < autoRetryDelaysMs.length; attemptIdx++) {
				if (this.indexingAbortController?.signal.aborted) {
					throw new Error('Indexing aborted by user');
				}

				const humanAttemptNumber = attemptIdx + 1; // 1-based
				if (this.progressCallback) {
				...
				this.embeddingAutoRetryAttempts = Math.min(autoRetryDelaysMs.length, this.embeddingAutoRetryAttempts + 1);
				if (connectionTest.success) {
					this.logIndexEvent('auto_resume', {
						attempt: humanAttemptNumber,
		...
		try {
			...
			if (filesToUpdate.length === 0) {
				...
				for (let i = 0; i < filesToUpdate.length; i++) {
					...
					if (file instanceof TFile) {
						if (this.progressCallback) {
							this.progressCallback(i + 1, filesToUpdate.length, `Indexing markdown file ${i + 1} of ${filesToUpdate.length}: ${file.basename}`);
						}
				...
				LoggingUtility.log(`Markdown file processing complete. Updated ${processedFiles} files.`);
			}
	...
	async forceCompleteRebuildIndex(progressCallback?: ProgressCallback): Promise<void> {
		...
		try {
			...
			for (let i = 0; i < files.length; i++) {
				...
				if (this.progressCallback) {
					this.progressCallback(i + 1, files.length, `Indexing markdown file ${i + 1} of ${files.length}: ${file.basename}`);
				}
			...
			LoggingUtility.log(`Markdown file rebuild complete. Indexed ${processedFiles} files.`);
		...
		try {
			...
			const chunkDocuments = chunks.map((chunk, index) => ({
				...
				metadata: {
					...
					anchorValue: chunk.anchorValue,
					anchorTarget: chunk.anchorTarget,
	...
	async search(query: string, limit: number = 5, threshold: number = 0.3): Promise<RAGSearchResult[]> {
		...
		// Search in unified vector database (catch dimension mismatch and notify user)
		let allResults: VectorSearchResult[] = [];
		try {
			allResults = this.vectorDB.search(queryEmbedding, limit, threshold);
		} catch (error) {
			LoggingUtility.warn('RAG search failed, falling back to keyword search:', error);
			const msg = error instanceof Error ? error.message : String(error);
			if (msg.toLowerCase().includes('dimension') || msg.toLowerCase().includes('vector dimension')) {
				new Notice('RAG search failed: index vector dimension mismatch. The index may have been built with a different embedding model. Please run "Force rebuild index" from the plugin settings or command palette.', 8000);
			} else {
				new Notice('RAG search failed: ' + msg, 8000);
			}
			return [];
		}
