...
export class ImageTextExtractor {
	...
	/**
	 * Enable or disable local OCR fallback (Tesseract).
	 * When disabled, the extractor will not attempt local OCR and will return an error
	 * if the model cannot perform vision extraction.
	 */
	setEnableLocalOCRFallback(value: boolean) {
		this.enableLocalOCRFallback = !!value;
	}
	...
	async extractTextFromImage(imageFile: TFile): Promise<ImageTextExtractionResult> {
		try {
			// Check whether the configured model supports vision
			const caps = await this.checkVisionCapabilities();

			// Read the image file as base64
			const imageData = await this.readImageAsBase64(imageFile);

			// If model doesn't support vision, optionally use local OCR fallback
			if (!caps.supportsVision) {
				LoggingUtility.log('Model does not support vision.');
				if (this.enableLocalOCRFallback) {
					LoggingUtility.log('Using local OCR fallback via Tesseract.');
					try {
						const ocrResult = await Tesseract.recognize(imageData, 'eng');
						const text = ocrResult?.data?.text?.trim() || '';
						if (!text) {
							return { success: false, extractedText: '', error: 'No text found in image (OCR fallback)' };
						}
			...
			// Attempt to send vision message to LLM (LLMService may attempt upload internally)
			try {
				...
				if (response.toLowerCase().includes('no text found') || response.toLowerCase().includes('cannot see')) {
					// Try OCR fallback once more
					try {
						const ocrResult = await Tesseract.recognize(imageData, 'eng');
						const text = ocrResult?.data?.text?.trim() || '';
						if (!text) {
							return { success: false, extractedText: '', error: 'No text could be extracted from the image' };
						}
						return { success: true, extractedText: text };
					} catch (ocrError) {
						LoggingUtility.error('Tesseract OCR failed after model response:', ocrError);
						return { success: false, extractedText: '', error: 'No text could be extracted from the image' };
					}
				}
				...
				LoggingUtility.warn('Vision message failed, attempting OCR fallback:', llmError);
				try {
					const ocrResult = await Tesseract.recognize(imageData, 'eng');
					const text = ocrResult?.data?.text?.trim() || '';
					if (!text) {
	...
	private async readImageAsBase64(imageFile: TFile): Promise<string> {
		try {
			...
			// Convert to base64 using chunked conversion to avoid call stack/argument limits
